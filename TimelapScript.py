...
parser = argparse.ArgumentParser(description='Timelaps script to use PiCamera Module.')
parser.add_argument('-t', '--time', metavar='\b', type=float, default=3600.0, help='Timelaps duration in seconds')
parser.add_argument('-i', '--interval', metavar='\b', type=int, default=2, help='Photo interval in seconds')
parser.add_argument('-f', '--folder', metavar='\b', default='', help='Location to store timelaps photos')
parser.add_argument('--version', action='version', version='TimelapsScript 0.1')
...
if (os.path.exists('./timelaps') == False):
    os.mkdir(folder)
...
''' 
sudo mencoder -nosound -ovc lavc -lavcopts vcodec=mpeg4:aspect=16/9:vbitrate=8000000 -vf scale=1920:1080 
